import { Network, Server, Database, Globe, MessageSquare, Shield, Crown, Cpu } from 'lucide-react'
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
        {COMPONENTS.map(comp => (
          <div
            key={comp.id}
            className={`rounded-xl border p-5 ${comp.color} transition-all hover:scale-[1.02]`}
          >
            ...
            {comp.connections.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {comp.connections.map(c => {
                  ...
                  return (
                    <span key={c} className="text-xs px-2 py-0.5 rounded-full bg-white/5 border border-white/10">
                      → {target?.label || c}
                    </span>
                  )
                })}
      ...
      <div className="mt-6 bg-roundtable-slate border border-roundtable-steel rounded-xl p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <h4 className="text-gray-400 mb-1">Frontend</h4>
            <p className="text-white">React 19 + TypeScript</p>
            <p className="text-gray-500">Vite, Tailwind CSS</p>
          </div>
          <div>
            <h4 className="text-gray-400 mb-1">API</h4>
            <p className="text-white">Go 1.23</p>
            <p className="text-gray-500">Gorilla Mux, WebSocket</p>
          </div>
          <div>
            <h4 className="text-gray-400 mb-1">Messaging</h4>
            <p className="text-white">NATS + JetStream</p>
            <p className="text-gray-500">Pub/Sub, persistence</p>
          </div>
          <div>
            <h4 className="text-gray-400 mb-1">Orchestration</h4>
            <p className="text-white">Kubernetes CRDs</p>
            <p className="text-gray-500">Custom operator</p>
          </div>
        </div>
      </div>
    </div>
